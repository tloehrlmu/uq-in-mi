...
def plot_accuracy_rejection_curve(true_labels, pred_labels, uncertainties, save_path, rand_order,
    ...
    plt.fill_between(portion_vals * 100,
                     np.array(acc_mean_rand_list) - np.array(acc_std_rand_list) * t_crit / np.sqrt(n),
                     np.array(acc_mean_rand_list) + np.array(acc_std_rand_list) * t_crit / np.sqrt(n), color='black',
                     alpha=0.2)
...
def hist_uncertainty(df, save_path, bins=11, score='TU_var', unc_label='TU_{var}'):
    p = sn.histplot(data=df, x=score, hue='correct', palette=['maroon', 'darkolivegreen'], bins=bins, stat='proportion',
...
def plot_uncertainty_average_groups(uncertainty_per_group, group_labels, save_path):
    plt.figure(figsize=(7, 6))
    plt.rcParams.update({'font.size': 24})
    # Sample data
    aleatoric_values = uncertainty_per_group[0]
    epistemic_values = uncertainty_per_group[1]
    total_values = uncertainty_per_group[2]

    # Bar width
    bar_width = 0.35

    # X-axis positions for the bars
    group_positions = np.arange(len(group_labels)) * 1.5

    # Create the bar plot
    plt.bar(group_positions - bar_width, aleatoric_values, width=bar_width, label='Aleatoric', color='red')
    plt.bar(group_positions, epistemic_values, width=bar_width, label='Epistemic', color='tab:blue')
    plt.bar(group_positions + bar_width, total_values, width=bar_width, label='Total', color='black', alpha=0.7)
