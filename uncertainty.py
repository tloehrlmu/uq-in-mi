...
# compute auroc
class_3_idx = np.array(y_true_class == 3)
uncertainty_ood = y_unc_mult_ens[class_3_idx, 0, :, 0, :]
uncertainty_id = y_unc_mult_ens[np.invert(class_3_idx), 0, :, 0, :]
labels = np.concatenate((np.zeros(uncertainty_id.shape[0]), np.ones(uncertainty_ood.shape[0])))
auroc_array = np.zeros((len(unc_types), num_ensembles))
for ensemble_index in range(num_ensembles):
    for idx, unc_type in enumerate(unc_types):
        uncertainties = np.concatenate(
            (uncertainty_id[:, idx, ensemble_index], uncertainty_ood[:, idx, ensemble_index]))
        auroc = roc_auc_score(labels, uncertainties)
        auroc_array[idx, ensemble_index] = auroc
        print(f'AUROC score {unc_type} uncertainty for ensemble {ensemble_index}: {auroc}')

print(f'AUROC score epistemic uncertainty: {np.mean(auroc_array[1, :])}+/-{np.std(auroc_array[1, :])}')
print(f'AUROC score aleatoric uncertainty: {np.mean(auroc_array[0, :])}+/-{np.std(auroc_array[0, :])}')

uncertainty_ood = np.mean(y_unc_mult_ens[class_3_idx, 0, :, 0, 0], axis=0)
uncertainty_id = np.mean(y_unc_mult_ens[np.invert(class_3_idx), 0, :, 0, 0], axis=0)
uncertainty_group = np.array([uncertainty_id, uncertainty_ood]).T
plotter.plot_uncertainty_average_groups(uncertainty_per_group=uncertainty_group,
                                        group_labels=['in-distribution', 'out-of-distribution'],
                                        save_path=f'{RESULTS_PATH}uncertainty_ood_avg_bar.pdf')
